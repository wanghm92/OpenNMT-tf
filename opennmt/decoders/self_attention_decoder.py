...
class SelfAttentionDecoder(decoder.Decoder):
  ...
  def _init_cache(self, memory, memory_sequence_length=None, dtype=tf.float32):
    ...
    cache = {
        "attn": tf.zeros([batch_size, 0, memory_time], dtype=tf.float32),
        "memory": memory,
    ...
    for l in range(self.num_layers):
      ...
      layer_cache = {
          "memory_keys": tf.zeros(proj_cache_shape, dtype=tf.float32),
          "memory_values": tf.zeros(proj_cache_shape, dtype=tf.float32),
      }
      if self.self_attention_type == "scaled_dot":
        layer_cache["self_keys"] = tf.zeros(proj_cache_shape, dtype=tf.float32)
        layer_cache["self_values"] = tf.zeros(proj_cache_shape, dtype=tf.float32)
      elif self.self_attention_type == "average":
        layer_cache["prev_g"] = tf.zeros([batch_size, 1, depth], dtype=tf.float32)
  ...
  def decode(self,
    ...
    if return_alignment_history:
      return (logits, None, None, sequence_length, attention)
  ...
  def dynamic_decode(self,
    ...
    if dtype is None:
      dtype = memory.dtype
    cache = self._init_cache(memory, memory_sequence_length=memory_sequence_length, dtype=dtype)
